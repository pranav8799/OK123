...
const Step2Documents: React.FC<Step2DocumentsProps> = ({
  ...
  const [documents, setDocuments] = useState<DocumentUpload[]>(() => {
    ...
    if (initialDocuments.length === 0) {
      return personalDocumentTypes.map(type => ({
        type,
        file: null,
        uploaded: false,
        required: ['Aadhaar', 'PAN', 'Cancelled Cheque'].includes(type)
      }));
    }
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  {requiredCount} required documents, {totalCount - requiredCount} optional
                </div>
              </div>
            </CardContent>
          </Card>
